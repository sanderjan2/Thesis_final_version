...
def logistical_regression(data):
    ...
    model = LogisticRegression(solver='lbfgs', max_iter=2200)
...
def Gradient_boosting(data, lrng_rate):
...
if __name__ == '__main__':
    ...
    for value in k_values:
        ...
        logistic_reg["Accuracy"][value] = logistical_regression(data)
        logistic_reg["k_value"][value] = value
        
        Grad_boosting["Accuracy"][value] = Gradient_boosting(data, learning_rate)
        Grad_boosting["k_value"][value] = value
        
        k_near["Accuracy"][value] = k_nearest(data, nearest_neighbors)
    ...
    for i, model in enumerate(model_dicts):
        ...
        df.to_csv("baseline/baseline_"+models[i]+".csv", index=False, header=True)
